...
public class Statistik
{
	...
	public Map<String, Integer> verbliebeneMerkmale(List< Schmetterling > lK, String m){
		...
		for (Schmetterling s : lK){
			if (s.getMerkmal().getMuster().equals(m)){
				String merkmale = s.getMerkmal().getFarbe() + " und " + s.getMerkmal().getForm();
				...
				String merkmale = s.getMerkmal().getMuster() + " und " + s.getMerkmal().getForm();
				if (sammelTrommel.containsKey(merkmale)){
					v = sammelTrommel.get(merkmale) + 1;
					sammelTrommel.put(merkmale, v);
					sTSize ++;
				...
				String merkmale = s.getMerkmal().getMuster() + " und " + s.getMerkmal().getFarbe();
				if (sammelTrommel.containsKey(merkmale)){
					v = sammelTrommel.get(merkmale) + 1;
					sammelTrommel.put(merkmale, v);
					sTSize ++;
	...
	public void outVerbliebeneMerkmale(Map map){
	    ...
	    while (it.hasNext()){
	        ...
	        double prozentGesammelt = (((Integer)pair.getValue()*100) /(double)sTSize) + 0.005;
	        String pGesammeltAsString = String.format ("%.2f", prozentGesammelt);
	        //System.out.println(pEAsString);
	        double prozentAlle = (((Integer)pair.getValue()*100) /(double)kinder.size()) + 0.005;
	        String pAlleAsString = String.format ("%.2f", prozentAlle);
	        
	        System.out.println(pair.getKey() + ": " + pair.getValue() + ", " + pGesammeltAsString + "% der eingesammelten Schmetterlinge und " + pAlleAsString + "% aller Schmetterlinge. ST:" + sTSize + ", Kinder:" + kinder.size());
	        it.remove();
	    }
	}
